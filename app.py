...
# [핵심] 4, 5, 7번 해결: 엔터 이동 + 숫자패드 + 0 자동 삭제/전체 선택 스크립트
components.html("""
    <script>
    const doc = window.parent.document;
    
    // 1. 숫자 칸 클릭 시 0 삭제 및 전체 선택 (5, 7번 해결)
    doc.addEventListener('focusin', function(e) {
        if (e.target.tagName === 'INPUT' && (e.target.type === 'number' || e.target.inputMode === 'numeric')) {
            ...
            // 혹은 기존 숫자가 있어도 바로 덮어쓸 수 있게 전체 선택
            e.target.select();
            e.target.dispatchEvent(new Event('input', { bubbles: true }));
        }
    });

    // 2. 엔터 키로 다음 칸 이동 (4번 해결)
...
search = st.text_input("🔍 물품 검색", placeholder="물품명을 입력하세요")

if not st.session_state.inventory.empty:
    df_m = st.session_state.inventory.copy()
    # 검색 필터
    filtered_items = [i for i in df_m['물품명'].unique() if search.lower() in i.lower()]
